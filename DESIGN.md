...
10. **除 `customPromptEnabled` 外一律不给 schema 默认值**：未设置 = `undefined` = 回落到组合配置/内置默认。
    「未配置时使用默认模型与默认系统提示词，且不报错」这条要求就落在 `effectiveConfig(config, undefined)`
...
### P7（系统提示词可见 / 追加提示词 / 恢复默认配置，2026-09-11）

44. **默认系统提示词"可见"靠下发一段唯一例外**：`/catalog` 增发 `defaults.systemPrompt`（内置/组合层的
    默认系统提示词）。规矩"提示词正文不下发"针对的是**宿主侧的默认/组合层文案**，而这一段本来就是
    部署方写进包里/配置文件的非私密内容，且"系统提示词在设置里可见可编辑"这条需求正需要它——
    否则没启用自定义的用户在设置页只能看到一只空输入框。设置页展开只读展示 + 「以默认为基础编辑」
    （填入正文并自动启用自定义开关：只填不启用会让人以为生效了）。顺带把"正文不下发"的边界写清楚：
    **用户自己存在设置段里的值**（自定义提示词、追加提示词正文、逐风格提示词）本来就随设置镜像回给设置页，
    这不与该规矩冲突——守卫钉住的是"宿主 index.js 不得自行把 `promptProfiles` 拼进 catalog"。
45. **追加提示词用"一个数组字段"而不是逐项字段**：`promptProfiles`（`{id,name,prompt}` 数组）+
    `activeProfileId`（启用项，空 = 不追加）两个字段就装下了"多条可切换"。这与风格的
    "逐项扁平字段"选择并不矛盾——风格清单要求**每个风格独立寻址**（字段名必须是静态的），
...
46. **取值与防御性回落**（P9 起：基底与追加分开算，见 50/51）：**基底**系统提示词 =
    ...
    整段设置失效；未启用任何条目时结果与没有这个功能之前**逐字节相同**（`profileId` 键都不出现，有用例钉住）。
    宿主 validate 拒绝：缺名称（内置条目除外）/缺正文（**全空行例外**——那是"刚点新增还没填"，客户端保存时丢弃）、
    id 重复、超 20 条、`activeProfileId` 指向不存在的条目（静默忽略会让用户以为它在生效）。
47. **切换 = 写一个字段，且必须写后自查**：`▾` 菜单点某条追加提示词 → 乐观更新选中标记 →
...
48. **精简/转规格从"独立的多选风格"变成"追加提示词清单里的内置条目"**：产品决策——两套并行的自定义体系
    （多选风格 + 提示词方案）让用户在设置页和 ▾ 菜单里各学一遍；合并后只剩一个清单、一种交互。
    实现上的关键是**保留旧语义作为默认文案**：内置条目未被覆盖时，它的正文就是那段追加要求原文
    （仍走 遗留设置字段 ← 组合配置同 id 预设 ← 内置文案 的老三层），拼装时才接在基底之后。
    于是"老部署选中精简"与旧多选**逐字节相同**（真框架集成测试钉住）。
49. **兼容面刻意保留了两处**：(1) 旧版浏览器半仍从 `/catalog.styles` 读多选清单、发 `styleIds`，
    宿主的追加路径与 `unknown-style` 校验原样保留（新客户端不再发，但字段照收）；
    (2) 遗留设置字段 `stylePromptConcise/stylePromptSpec` 仍是内置条目追加文案的最高层——
    升级后老配置继续生效。对应的代价是：内置条目的存储覆盖条目**允许省略 name**
    （显示名由宿主按内置标签补齐）——客户端不应把随界面语言变化的名称写进存储，
    这条豁免写进了 `validateSettingsSection` 与客户端 buildOps（内置覆盖只发 `{id, prompt}`）。

### P9（命名与语义对齐：系统提示词 / 追加提示词，2026-09-11）

50. **旧名说的是两件说不清的事**：「提示词」与「提示词方案」两个词都含"提示词"，
    用户在设置页里看不出谁是基底、谁是附加（本次对话里就真实发生过一次误解）。
    改名后关系写进名字：**系统提示词**（基底，永远发给模型）+ **追加提示词**（选中哪条就接在基底之后）。
51. **改名同时暴露了一处语义不诚实**：P7/P8 里"选中一条方案"是**整体替换**系统提示词，
    而名字叫"追加"就要求它是**追加**。P9 把语义对齐为追加——这也让两条路径收敛到同一个格式：
    内置条目选中（`activeProfileId`）与旧客户端 `styleIds` 拼出的 system prompt **逐字节相同**
    （都是 `基底\n\n本次额外要求（label）：正文`），对照排查时不必再区分来源。
    实现落点是分层：`effectiveConfig` 只算**基底**（追加条目不再参与 `systemPrompt`，
    `sources.prompt` 因此只标基底那一层），追加条目在拼装期由 `systemPromptFor(resolved, presetId, styles, append)`
    接上；`activeProfileId` 命中的条目由 `prepareCall` 从 `effective.profiles` 取出后传入。
    代价写进了 README：追加语义下不能"按条目整段换掉系统提示词"——要换就改「系统提示词」区块
    （全局生效），或把那段话作为追加条目写进去（追加内容在指令序列里更靠后，实际影响通常更大）。

### P10（设置界面重设计：默认收起、压行距、一屏看全，2026-09-11）

52. **"输入框默认隐藏"用"不渲染"实现，而不是 CSS 隐藏**：每个分组一行「标题 + 只读摘要 + 编辑」，
    展开态才 `createElement` 出 input/textarea（收起时它们根本不在 DOM 里）。这样没有多余的 DOM、
    不会被 tab 焦点或密码管理器误触，也让"默认不可见"这件事可以被**结构断言**钉住
    （用例直接查 `inputs.size === 0` 与 `textareas.length === 0`），而不是查 CSS 规则。
    展开态存在组件 state（`expanded`：`prompt`/`model`/`params`/`profile:<id>`），按需独立展开。
53. **密度是按"一屏塞下全部分组"倒推的**：正文 12px/行高 1.45、分组间距 8px、组头 24px、清单行 22px、
    按钮 11px——旧版式是 13px 正文 + 14px 内边距的 fieldset + 18px 间距，屏上只放得下两个分组。
    同时把**操作条搬到顶部**（保存/恢复默认/打开配置文件常驻），用户不必滚到底部找按钮；
    每个分组的摘要行承担"收起时的信息量"（自定义 · n 字 / acme / m1（设置页）/ 温度 默认 · 上限 1024 · 超时 30000ms）。
    旧版式的 CSS 类（`dsh-bi-fieldset`/`dsh-bi-legend`/`dsh-bi-grid`）与旧尺寸字面量一并删掉，
    并加了一条用例**反查**它们不再出现——避免两套密度混着用慢慢长回来。
54. **折叠带来一个真实的新失败面：错误提示被藏起来**。"有字段不合规，未保存"如果只显示在顶部，
    而出错的分组正收起着，用户就找不到是哪个字段。所以保存前校验失败时先 `groupsWithErrors(validation)`
    把出错的分组与出错的行一并展开（纯函数、可单测），再给提示。这条比"直接暴露输入框"更值得做：
    紧凑布局的全部收益都建立在"收起态是安全的"这个前提上。
55. **功能零回归的验证方式**：没有靠"看起来没变"，而是把既有断言逐条迁到新布局上——需要输入的用例
    先显式 `open('prompt')`（替身里的展开开关就是真按钮），行为断言（ops 内容、revision、
    写后自查、错误文案、重置清单、只读/不可用两态）一字不改。真 React SSR 套件改为断言
    **默认视图**的结构（无输入框、四个分组、清单三条、操作条齐全），替身套件则覆盖展开后的交互。

---

### P11（执行中可见：进度行 + 思考过程面板，2026-09-11）

56. **要解决的其实不是"缺动画"，而是"缺事实"**：P5.6 的流式回填让**文本**边到边写，但从"请求发出"
    到"第一个文本 token"之间没有任何事件——推理模型这一段可以十几秒（默认超时 30s、上限可到 600s），
    界面上只有一个静止的图标。所以补的是两样可核对的事实：**阶段**（在等模型 / 在想 / 在写）、
    **已耗时**、**已收字数**；顺带把按钮图标改成透明度呼吸（`@keyframes dsh-bi-pulse`），
    因为静止图标本身就是"卡死"观感的一部分。
57. **进度帧不做，阶段由客户端推导**：宿主只多转一种 `reasoning` 事件，**没有** `progress` 事件、
    没有心跳帧。理由：等待期一个事件都没有，本地时钟才是唯一可用的进度来源（心跳帧解决的是"连接是否
    活着"，而那条已经被 fetch/abort 覆盖）；阶段用"收到了哪类增量"推导（首个 reasoning → thinking，
    首个 text-delta → writing），比宿主额外维护一份状态机少一处会漂移的契约。
    **不给百分比**：`maxOutputTokens` 是上限不是目标，任何比例都是编的数字——进度可以是不确定型的，
    但不能是假的。计数用**字数**而非 token：客户端自己就算得准。
58. **思考增量的开关放在宿主侧，语义是"不发"而不是"不显示"**：`showReasoning` 进设置命名空间
    （`z.boolean()`、**未设置 = 默认开**、只有显式 `false` 才关，与"未设置就回落到默认"同规矩），
    `prepareCall` 每次请求现读，关掉时 `ctx.llm.stream` 的 reasoning 增量**根本不写进 SSE**。
    思考正文（模型的推理内容）因此不出宿主，而不是"发到浏览器再藏起来"。
    客户端不参与这个判断（少一处两端会不一致的判据），`/catalog` 的 `effective` 里带上它纯粹为了"生效值可查"。
    另有一条守卫钉住"`reasoning-delta` 必须与 `showReasoning` 出现在同一行"。
59. **思考文本与草稿之间必须有结构性的隔离，而不是靠自觉**：
    · 宿主侧：权威文本仍旧只由 `.filter(block => block.type === 'text')` 装配——思考块在**类型层面**
      就进不了 `done.text`；
    · 客户端：`write()` 只被文本增量与 `done` 触发，思考增量走独立的 `onReasoning` 支路，
      只进展示 state（另有守卫禁止 `setDraft(thinking…)`）；
    · 撤销栈里因此只有"优化前 → 优化后"的草稿。
    三条各自可断言，合起来覆盖"推理内容被写进用户输入框"这条最不能接受的退化。
60. **面板的生命周期**：出现第一段思考即自动展开（只自动展开**一次**——用户手动收起后不该被下一段增量顶开），
    收尾（成功/失败/取消都一样）自动收起并在按钮行留一个思考图标回看。**失败也留内容**：
    草稿照旧还原成原文，但思考留着——失败原因常常就写在思考里，丢掉等于把唯一的线索扔了。
    与 `▾` 预设菜单**互斥**（两者锚在同一处，同时打开会叠在一起）。
61. **两处 100ms 节流，且都不丢内容**：思考增量逐 token 到达，每个都 setState 会让组件每秒重渲染上百次。
    节流只作用于"同一阶段内的计数滚动"，**阶段切换与收尾一律立即落地**；收尾那次会把节流期间攒下的
    文本一并 setState（局部量里从未丢过），所以面板最多滞后 100ms，不会漏字。
    超长时保留**尾部** 20000 字（生成中要跟着最新思考走），被截断时正文开头明确写一句，
    标题里的累计字数仍显示总数。
62. **测试怎么钉住"时间"**：进度行带耗时、面板走节流，两者都读 `Date.now()`——用例里用一个
    `withClock` 把时钟冻结再显式推进（`advance(2400)` + `tickIntervals()`），
    于是"2.4s""节流窗口内的增量先攒着、到点再落地"都是确定性的断言，而不是"跑得快不快"的运气。
    测试替身也补了 `window.setInterval/clearInterval`（登记回调、由用例推进）：缺了它们，
    代码里的 `window.setInterval?.` 会静默短路，"计时器到底起没起"整个漏掉。

---

### P12（控件收敛：工具行只留 ✦ 与 ▾，2026-09-11）

63. **问题不在"多了一个按钮"，而在"最坏情况下四个同时在场"**：P11 之后，一次带思考的成功优化会同时
    挂上 撤销 / 思考 / ✦ / ▾ 四个控件，再叠加进度（或提示）行与弹出的面板；而这一行还要放附件、
    模型选择器、上下文计量与发送键（`gap:6px`、每个按钮 24px 起、提示槽位最多 240px）。
    拥挤感来自最坏态，而不是平均态——所以优化目标是**最坏态**。
64. **收敛策略：把"次要功能"整体搬进已有的 ▾ 菜单，而不是新造入口**。菜单本来就有两个分区
    （追加提示词 / 预设）与既成手势（向上弹出、点外面或 Esc 收起、点项不收起），
    新增一个「本次调用」分区是**加内容**而不是加控件。结果：工具行恒定 2 个控件，
    且**宽度不再随"有没有撤销/有没有思考"跳动**——布局稳定比"少两个按钮"更值钱。
    菜单里按"什么时候有用"排序（本次调用 → 长期设置），而不是按实现层次。
65. **▾ 的显示条件必须与 `/catalog` 解耦**：撤销与思考回看住进菜单之后，若菜单仍只由
    `profiles`/`presets` 决定是否渲染，那么目录读取失败（离线、宿主半缺失）时菜单整个消失，
    一次成功的优化之后就再也找不到撤销入口，而主按钮看起来一切正常。
    现在的条件是 `hasInvocation || profiles.length > 0 || oneShotPresets.length > 0`，
    并有一条守卫禁止退回旧写法。这是"把功能搬进容器"最典型的隐藏耦合。
66. **两步确认要能在菜单里成立，所以 `onUndo()` 改成返回布尔值**：执行了还原 → 收起菜单；
    只是"武装"了强制还原 → **菜单保持展开**并把菜单项文案换成「↶ 强制还原原文」，
    用户不必重开菜单就能点第二次。旧实现是独立按钮，从来没有"点完菜单会不会关"这个问题；
    文案随状态切换也比原来那句 flash 更清楚（状态就长在操作对象上）。
67. **顺手修掉的脚枪**：撤销以前是独立按钮且**生成中仍可点**，点它会 `setDraft` 打断正在跑的那次
    流式调用（流式 CAS 判成"用户手改"，结果被丢弃）。现在撤销在菜单里，而菜单在生成中本来就是禁用的
    （`disabled: running || disabled`）——同一个改动顺手关掉了一个真实的误操作路径。
68. **成功不再弹提示**：草稿被替换是肉眼可见的，再闪一句「已替换为优化结果」属于噪声；
    "上次优化还能撤销"改由 ▾ 右上角的 5px 圆点表达（角标不承担点击，只表达"有东西可撤"）。
    保留提示的只有**界面上看不出来**的那几类：截断、失败、已还原原文、草稿已变化。
    这条把提示槽位的出现频率砍掉大半，是"降低视觉复杂度"里最省力的一半。
69. **菜单项用文字前缀（`↶` / `💭`），不引入图标**：菜单项本来就以文字为主，塞 14px 图标既要处理
    对齐，又要给每个图标多留一条"图标缺失"的降级路径；文字前缀在任何图标集下长得一样。
    代价是删掉了 `IconUndo`/`IconThink` 两个常量（不做无用的兼容分支）。
70. **测试怎么钉住"收敛"这件事**：新增一条结构断言——一次带思考的成功优化之后，
    `wrap` 的直接子按钮**必须恰好是** `[optimize, menu]`，且菜单项顺序必须是
    `[本次调用, 撤销, 思考, 追加提示词, …]`。这比"某两个 data 属性还在不在"更接近验收标准
    （旧版这里是 4 个按钮）。替身也补了 `openMenu()` / `clickUndo()`：**不替用例偷偷开菜单**，
    否则"菜单渲染不出来"这类回归永远看不见；多会话用例里的"没有撤销记录"断言必须开菜单后再判空，
...
### P14（思考强度：归在「调用参数」里的一个默认值，2026-09-13）

81. **"思考强度"就是 dsh-llm 的 `GenerateOptions.reasoningEffort`**：它是**适配器所有的不透明字符串**
    （README 明写 "adapter-owned opaque strings resolved only against each adapter's advertised set"），
    取值集合与默认值都随模型而变。所以设置层不做白名单，只要求非空字符串；可用性交给宿主在调模型前核对
    （第 83 条）。设置页的 datalist 只是**建议**（宿主 `/catalog` 的 `limits.reasoningEfforts` 下发，
    离线时兜底一份并在测试里与宿主常量对拍），用户填别的值一样合法。
82. **只有一个字段，而且它是一个调用参数**（`defaultReasoningEffort`，放在设置页「调用参数」分组里）。
    用户的后续要求是"不需要配置特定模型的默认强度"，所以**刻意不做按模型覆盖**：
    - "某个模型支不支持某个强度"是适配器的事实，不是用户该维护的对照表；真按模型配，用户就得先知道
      每个模型公布哪些 id，配错还没人告诉他——而宿主本来就能在调模型前问适配器（第 83 条）。
    - 一个 `provider + model` 的对照表还会在换模型后过期，而它过期时不会有任何提示。
    内置默认是需求指定的 `low`——"未配置"与"配置成 low"因此同效，设置文档保持最小（与 `showReasoning`
    相反方向的同一条规矩：未设置就回落，不往文档里写一个恰好等于默认值的值）。
83. **必须做能力核对，不能直接透传**（这是本期最容易踩的坑）：dsh-llm 的 `resolveCallWithInfo` 对
    "模型不公布 reasoning 却收到显式 `reasoningEffort`"以及"值不在公布集合里"都会**抛
    `UNSUPPORTED_REASONING_EFFORT`**。而默认值 `low` 是对**所有**模型生效的——直接透传会把
    "不支持思考强度的模型"整条打成 502，一个纯设置项不该有这种杀伤力。所以 `lib/index.js` 的
    `createReasoningEffortResolver` 先 `ctx.llm.resolveModelInfo()` 拿 `reasoning.efforts`：
    公布且包含该值才传；否则**省略**（回到适配器自己的默认 = 加这个功能之前的行为）。
    这条有专门的约定守卫（`check-guards.mjs`）钉住。
84. **降级要分两种，日志也要分两种**：模型压根不公布思考强度 = 能力事实（`info`，每个模型一条，
    不刷屏）；模型公布了集合但没有配置值 = 配置与能力不匹配（`warn`，用户需要知道设置没生效）。
    能力查询自身失败**不当失败**处理：按"不支持"省略，真正的原因由随后的 `stream` 如实抛出——
    在能力查询这一步编一个错误，只会把一个可能能跑通的调用提前打死。查询结果按 `provider/model`
    缓存（上限 200 条，插件重载即清零），免得每次优化都多一次适配器 I/O。
85. **核对排在并发闸门占位之前**：能力查询是异步 I/O，占着名额去等会把闸门拉长；而它又不会抛
...
**红线**：
- ❌ 不要 `querySelector` 改输入框 DOM。已安装版本的 composer 是 Lexical contenteditable + 芯片节点（`lib/types/client/input/editor/ComposerContentEditable.d.ts`、`chip-node.d.ts`），DOM 改法会被下一次渲染冲掉，还会绕过输入机状态机。（本地源码检出的 `InputBar.tsx` 已改为 `textarea + backdrop` 追加提示词——**两版都靠同一套 `draft`/`setDraft` 契约**，这正是不要碰 DOM 的理由。）
