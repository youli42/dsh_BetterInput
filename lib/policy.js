...
/**
 * 内置**优化风格**：输入框下拉框可多选，每一项的提示词都能在设置页逐项覆盖。
 *
 ...
 * 所以风格清单固定在代码里，**提示词**走三层覆盖（内置 ← 组合配置同 id 预设 ← 设置页），
 * 这样既满足"每个风格单独配提示词"，又让老部署的 `presets` 继续生效。
 */
...
/**
 * 拼装本次调用的 system prompt：基础提示词 + 选中的风格 + 预设（一次性）。
 *
 * 顺序刻意固定（基础 → 风格 → 预设）：风格是"多选、可叠加"的长期偏好，预设是"本次额外"
 * 的一次性附加，放在最后让模型按最近的要求收尾。
 * @param {Readonly<object>} resolved - resolveConfig 的产物。
 * @param {string | undefined} presetId - 请求指定的预设 id。
 * @param {ReadonlyArray<{ label: string, prompt: string }>} [styles] - 本次选中的风格（已解析出提示词）。
 * @returns {string} system prompt。
 * @throws {RequestError} presetId 不在组合配置的预设里时抛 400。
 */
export function systemPromptFor(resolved, presetId, styles) {
