 ...
 *
 * 需求：设置里能指定「优化时用多大的思考强度」。它只有一个**默认值**（对所有模型生效），
 * 归在「调用参数」一栏里——刻意不做"按模型分别配置"：那会让设置页多出一张要用户自己维护的
 * 对照表，而"某个模型支不支持某个强度"本来就是适配器的事（见下）。
 *
 * 语义要点：
 *   · 强度 id 是**适配器所有的不透明字符串**（dsh-llm README："Reasoning-effort identifiers
 *     are adapter-owned opaque strings resolved only against each adapter's advertised set"），
 *     所以设置层不写死白名单，只要求"非空字符串"；真正的可用性由宿主在调模型之前用
 *     `ctx.llm.resolveModelInfo()` 拿到的 `reasoning.efforts` 核对（见 lib/index.js）。
 *   · 内置默认是 `low`（需求明写）。"未配置" 与 "配置成 low" 因此同效，设置文档保持最小。
 */

/** 设置段里"默认思考强度"的字段名（调用参数之一）。 */
export const DEFAULT_REASONING_EFFORT_FIELD = 'defaultReasoningEffort'
/** 内置默认思考强度：需求指定为 low。 */
export const DEFAULT_REASONING_EFFORT = 'low'
/**
 * 设置页 datalist 用的**建议**强度 id（不是白名单）。
 *
 * 强度集合归适配器所有，这里只是让常见取值可以点选；用户填别的值一样合法，
 * 最终由模型公布的能力决定用不用得上。
 */
export const REASONING_EFFORT_SUGGESTIONS = Object.freeze(['minimal', 'low', 'medium', 'high'])
...
export function validateSettingsSection(section) {
  ...
  // 思考强度：默认值必须是文本（空串 = 放弃覆盖，回落内置默认 low）；非空即合法——
  // 强度 id 归适配器所有，这里不做白名单（可用性由宿主按模型公布的能力核对）。
  if (value[DEFAULT_REASONING_EFFORT_FIELD] !== undefined && value[DEFAULT_REASONING_EFFORT_FIELD] !== null
    && typeof value[DEFAULT_REASONING_EFFORT_FIELD] !== 'string') {
    errors.push('思考强度必须是文本')
  }
...
export function effectiveConfig(config, section) {
  ...
  /**
   * 思考强度（P14）：设置里的 `defaultReasoningEffort`，未设置 = 内置 `low`。
   * 它是对**所有**模型生效的一个调用参数（不做按模型覆盖）；真正用不用得上由宿主按
   * 模型公布的能力核对（见 lib/index.js），因为强度 id 归适配器所有。
   */
  const reasoningEffort = isFilledText(value[DEFAULT_REASONING_EFFORT_FIELD])
    ? /** @type {string} */ (value[DEFAULT_REASONING_EFFORT_FIELD]).trim()
  ...
  return {
    ...
    // 具体某个路由用不用得上，由宿主按模型公布的能力核对（见 lib/index.js）。
    reasoningEffort,
    styles,
