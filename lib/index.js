...
/** 路由路径（浏览器半镜像同一组字面量）。 */
export { ROUTE, ROUTE_CATALOG, ROUTE_CATALOG_MODELS, ROUTE_CHECK }
...
function replyFailure(ctx, response, error, code) {
  ...
  logger(ctx).warn(`better-input: ${code}: ${message}`)
 ...
 */
function createOptimizeHandler(ctx, resolved, settings) {
  ...
  return async function handler(request, response) {
    ...
    try {
      ...
      const result = await optimizeText(ctx, effective, {
        ...route,
        system,
        text,
      ...
      const timedOut = timeout !== undefined && timeout.aborted && !client.signal.aborted
 ...
 * @returns {(request: object, response: object) => Promise<void>} 处理器。
 */
function createReadHandler(ctx, method, run) {
  return async function handler(request, response) {
    try {
      ...
      replyFailure(ctx, response, error, 'catalog-failed')
    }
  }
}
...
async function checkPayload(ctx, request) {
  ...
  try {
    ...
    return {
      ...
      ...typeof info?.context === 'number' ? { context: info.context } : {},
...
export function apply(ctx, config) {
  ...
  // 设置是可选依赖：注册失败（例如外部手改出的非法段）只降级，绝不拖垮基础功能。
  let settings = { available: false, read: () => undefined }
  try {
    settings = openSettings(ctx)
  } catch (error) {
    log.warn(
      `better-input: settings namespace unavailable: ${error instanceof Error ? error.message : String(error)}`,
    )
  }
  const routes = [
    { path: ROUTE, handler: createOptimizeHandler(ctx, resolved, settings) },
    {
      path: ROUTE_CATALOG,
  ...
  log.info(
    `better-input: mounted ${ROUTE} (+catalog/check)${settings.available ? '' : ' [settings unavailable]'}`,
  )
}
