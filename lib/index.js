...
/** 路由路径（浏览器半镜像同一组字面量）。 */
export { ROUTE, ROUTE_STREAM, ROUTE_CATALOG, ROUTE_CATALOG_MODELS, ROUTE_CHECK }
 ...
 * @returns {Promise<{ text: string, presetId?: string, sessionId?: string, route: { provider: string, model: string },
 *   limits: object, system: string, timeoutMs: number, slot: object }>} 调用参数（slot 需在 finally 释放）。
 */
async function prepareCall(ctx, resolved, settings, request, gate) {
  ...
  return {
    ...
    limits: effective,
    system: systemPromptFor({ systemPrompt: effective.systemPrompt, presets: resolved.presets }, presetId),
    route: resolveRoute(ctx, effective),
 ...
 * @returns {(request: object, response: object) => Promise<void>} 处理器。
 */
function createReadHandler(ctx, method, run, requirement = { session: false }) {
  return async function handler(request, response) {
    try {
...
export function apply(ctx, config) {
  ...
  log.info('better-input: mounted %s (+stream/catalog/check)', ROUTE)
}
