 ...
 *   reasoningEffort?: string, sessionId?: string, signal: AbortSignal }} call - 本次调用参数。
...
async function optimizeText(ctx, limits, call) {
  ...
  const options = {
    ...
    // 思考强度（P14）：只在适配器确认模型公布了这个强度时才存在（见 prepareCall 的核对）。
...
/**
 * 思考强度（P14）的能力核对器：把配置层解析出的强度与**模型真正公布的能力**对齐。
 *
 * 为什么不能直接把设置里的强度塞进 `ctx.llm.stream()`：dsh-llm 的 `resolveCallWithInfo`
 * 对"模型不公布 reasoning 却收到显式 reasoningEffort"以及"值不在公布集合里"都是**抛错**
 * （`UNSUPPORTED_REASONING_EFFORT`）。而本功能的默认值 `low` 是**对所有模型**生效的，
 * 直接透传会把"不支持思考强度的模型"整条打成 502——一个纯设置项不该有这种杀伤力。
 * 所以这里先问适配器：模型公布了该强度才传；没公布/不支持就**省略**（回到适配器自己的默认，
 * 与加这个功能之前完全一致），并留一条日志说明"设置生效不了"。
 *
 * 查询结果按 `provider/model` 缓存（适配器能力在插件生命周期内基本不变，而能力查询可能是一次
 * 异步 I/O）；插件重载/卸载时缓存随之清零。查询本身失败**不致命**：当作"不支持"处理，
 * 真正的原因会由随后的 stream 如实抛出，而不是在这里提前编一个错误。
 * @param {object} ctx - 宿主上下文。
 * @param {{ info: Function, warn: Function }} log - 日志门面。
 * @returns {(provider: string, model: string, requested: string | undefined) => Promise<string | undefined>} 核对器。
 */
function createReasoningEffortResolver(ctx, log) {
  /** 缓存条目：模型公布的强度 id 集合（空数组 = 模型不公布思考强度 / 查询失败）。 */
  const cache = new Map()
  /** 上限：模型 id 是有限集合，但设置可以被反复改，缓存不设上限就会单向长大。 */
  const MAX_CACHE_ENTRIES = 200
  return async function resolveEffort(provider, model, requested) {
    ...
    if (entry === undefined) {
      ...
      try {
        ...
        // 能力查询失败：不缓存结论可能也不对，但更不该在这里拒绝调用。
        log.warn(
          'better-input: reasoning-effort probe failed for %s/%s: %s',
          provider,
          model,
          error instanceof Error ? error.message : String(error),
        )
      }
      // why 记在条目里：同一条"设置生效不了"的告警只打一次，免得每次优化都刷一条。
      entry = { efforts, warned: false }
      if (cache.size >= MAX_CACHE_ENTRIES) cache.clear()
      cache.set(key, entry)
    }
    if (effortsUnsupported(entry.efforts, requested)) {
      if (!entry.warned) {
        entry.warned = true
        if (entry.efforts.length === 0) {
          // 模型压根不公布思考强度：这是能力事实，不是配置错误——一条 info 即可，
          // 免得每个不支持推理的模型每次优化都刷一条告警。
          log.info(
            'better-input: model %s/%s does not support reasoning effort; ignoring "%s"',
            provider,
            model,
            requested,
          )
          ...
          log.warn(
            'better-input: model %s/%s does not offer reasoning effort "%s" (offered: %s); using the adapter default',
            provider,
            model,
            requested,
            entry.efforts.join(', '),
...
/**
 * 请求的强度是否不在模型公布的集合里（集合为空 = 模型不公布思考强度，同样算用不了）。
 * @param {string[]} efforts - 模型公布的强度 id。
 * @param {string} requested - 配置里解析出的强度。
 * @returns {boolean} 是否用不了。
 */
function effortsUnsupported(efforts, requested) {
 ...
 * @param {(provider: string, model: string, requested: string | undefined) => Promise<string | undefined>} resolveEffort - 思考强度能力核对器。
 ...
 *   showReasoning: boolean, reasoningEffort?: string, slot: object }>} 调用参数（slot 需在 finally 释放）。
 */
async function prepareCall(ctx, resolved, settings, request, gate, resolveEffort) {
  ...
  // 下面三处都会抛（未知预设 400 / 没有模型路由 502），所以**必须**排在占位之前：
  // 闸门的名额是"已占用"就没人还的，占位后抛错会让该会话之后恒 409、累计满额后全局恒 429，
  // 只能靠重启宿主恢复。而"没配模型/还没选过默认模型"正是新装首用的常见状态。
  // 启用中的追加提示词（activeProfileId 命中的那条）在这里接上基底——基底永远在场，追加只增不改。
  const activeAppend = effective.profileId !== undefined
    ? effective.profiles.find(profile => profile.id === effective.profileId)
    : undefined
  const system = systemPromptFor(
    { systemPrompt: effective.systemPrompt, presets: resolved.presets },
    presetId,
    styles,
    activeAppend !== undefined ? { label: activeAppend.label, prompt: activeAppend.prompt } : undefined,
  )
  const route = resolveRoute(ctx, effective)
  // 思考强度（P14）：生效配置里那一个默认值（未设置 = 内置 low），再问适配器模型是否公布它。
  // 位置刻意排在闸门占位**之前**：能力查询是一次异步 I/O，占着名额去等会把闸门拉长。
  const reasoningEffort = await resolveEffort(route.provider, route.model, effective.reasoningEffort)
  ...
  return {
    ...
    // 思考过程是否透传（P11）：由**生效设置**在每次请求时现读决定，客户端不参与这个判断——
    // 关掉时思考正文根本不离开宿主，而不是"送到了浏览器再藏起来"。
    showReasoning: effective.showReasoning !== false,
    // 思考强度（P14）：只有适配器确认这个模型公布了该强度才带上（见 createReasoningEffortResolver）。
 ...
 */
function createOptimizeHandler(ctx, resolved, settings, gate, resolveEffort) {
  ...
  return async function handler(request, response) {
    ...
    let slot
    try {
      const call = await prepareCall(ctx, resolved, settings, request, gate, resolveEffort)
      slot = call.slot
 ...
 * （共用 `prepareCall`）。增量只转发**文本增量**（`delta`）与**思考增量**（`reasoning`，
 * P11 起、仅在生效设置开着"显示思考过程"时），最终一帧 `done` 带装配后的权威文本 ——
 * 客户端以 `done` 的文本为准，避免增量与装配结果在边界上不一致。
 * @param {object} ctx - 宿主上下文。
 * @param {Readonly<object>} resolved - 已校验的插件组合配置。
 * @param {{ available: boolean, read: () => unknown }} settings - 设置门面。
 * @param {{ acquire: (sessionId: string | undefined) => object }} gate - 并发闸门。
 ...
 * @returns {(request: object, response: object) => Promise<void>} 处理器。
 */
function createStreamHandler(ctx, resolved, settings, gate, resolveEffort) {
  const log = logger(ctx)
  return async function handler(request, response) {
    const client = clientAbortSource(response)
    let timeout
    ...
    try {
      ...
      for await (const chunk of ctx.llm.stream({
        ...
        signal,
        ...call.limits.temperature === undefined ? {} : { temperature: call.limits.temperature },
        // 思考强度（P14）：与一次性路由同一份核对结论（prepareCall 里算好）。
        ...call.reasoningEffort === undefined ? {} : { reasoningEffort: call.reasoningEffort },
        ...call.sessionId === undefined ? {} : { sessionId: call.sessionId },
      })) {
        assembler.push(chunk)
        if (chunk.type === 'text-delta') stream.send('delta', { text: chunk.text })
        // 思考增量（P11）：只在用户开着「显示思考过程」时转发。1:1 转发而不是在宿主节流——
        // 宿主的成本只是写一帧 SSE，真正的渲染成本在浏览器侧（客户端按 PROGRESS_MS 节流）。
        // **装配照旧只取 text 块**：思考内容结构上不可能进 done 的权威文本或草稿。
        else if (chunk.type === 'reasoning-delta' && call.showReasoning) {
          stream.send('reasoning', { text: chunk.text })
        }
      }
      const outcome = finishOutcome(assembler.finish)
      if (outcome.error !== undefined) throw outcome.error
 ...
 * 而不是把宿主侧提示词回灌到浏览器。
 *
 * 追加提示词（`profiles`）走同一个规矩：行由 `profileRowsOf` 生成，只含 id/名称/来源/是否内置；
 * 追加提示词的正文是用户存在设置段里的值，客户端要编辑时从自己的设置镜像里拿，不从这条路由拿。
 * 内置优化风格（精简/转规格）已并入追加提示词清单（内置种子），同时 `styles` 仍按原样下发——
 * 旧版浏览器半（多选风格）靠它继续工作。
 *
 * `defaults.systemPrompt` 是**有意**下发的唯一一段提示词正文：内置/组合层的默认系统提示词
 * 不是用户私密（部署方写在本包配置里），而「系统提示词在设置里可见」正需要它——
 * 用户没启用任何自定义时，设置页就展示这一段，并可以"以它为基础"去编辑。
 * @param {object} ctx - 宿主上下文。
 * @param {Readonly<object>} resolved - 组合配置。
...
function catalogPayload(ctx, resolved, settings) {
  ...
  return {
    ...
    limits: {
      ...
      // 思考强度的**建议**清单（不是白名单）：设置页的 datalist 用它，
      // 用户填别的值一样合法——可用性最终由模型公布的能力决定。
      reasoningEfforts: REASONING_EFFORT_SUGGESTIONS,
    },
    ...
    // 追加提示词行（内置种子 + 用户条目）：id/名称/来源/是否内置，
    // prompt 绝不在这条响应里——由 profileRowsOf 的实现保证。
    profiles: profileRowsOf(effective),
    defaults: {
      systemPrompt: resolved.systemPrompt,
      // 内置默认思考强度（未配置时用的那个）——设置页据此显示"未设置就用它"。
    ...
    effective: {
      ...
      timeoutMs: effective.timeoutMs,
      // 思考过程是否透传（P11）：默认 true，只有设置页把它关掉才是 false。
      // 下发它是为了"生效值可查"——排查"为什么看不到思考"时不用去翻 settings.yaml。
      showReasoning: effective.showReasoning !== false,
      // 思考强度（P14）：对所有模型生效的调用参数，永远有值（未设置 = 内置 low）。
      // 注意它**不保证**模型真的会用：模型没公布这个强度时宿主会省略并留日志。
      reasoningEffort: effective.reasoningEffort,
      // 当前启用的追加条目 id（null = 没启用任何追加提示词，走默认提示词链）。
...
async function modelsPayload(ctx, request) {
  ...
  return {
    ...
    models: (models ?? []).map(model => ({
      ...
      // 思考强度（P14）：适配器公布就一并下发（设置页可以据此给出该模型可用的强度）。
      // 目录是增强信息，形状不认识就当没有——绝不能因为少一个字段让列表整条失败。
      ...reasoningRow(model.reasoning),
    })),
  }
}

/**
 * 把适配器公布的 reasoning 元数据裁成设置页用得上的形状（不认识就当没有）。
 * @param {unknown} reasoning - `LlmModelInfo.reasoning`。
 * @returns {{ efforts?: string[], defaultEffort?: string } | {}} 可下发的行（或空对象）。
 */
function reasoningRow(reasoning) {
  if (reasoning === null || typeof reasoning !== 'object') return {}
  const efforts = Array.isArray(reasoning.efforts)
    ? reasoning.efforts.map(effort => effort?.id).filter(id => typeof id === 'string' && id !== '')
    : []
  if (efforts.length === 0) return {}
  return {
    efforts,
    ...typeof reasoning.defaultEffort === 'string' && reasoning.defaultEffort !== ''
...
async function checkPayload(ctx, request) {
  ...
  try {
    ...
    return {
      ...
      // 思考强度（P14）：这个模型公布的强度与它自己的默认值（没有就不带）。
      ...reasoningRow(info?.reasoning),
    }
  } catch (error) {
...
export function apply(ctx, config) {
  ...
  // 思考强度（P14）的能力核对器：带短期缓存，随 fiber 生命周期。
  const resolveEffort = createReasoningEffortResolver(ctx, log)
  const routes = [
    { path: ROUTE, handler: createOptimizeHandler(ctx, resolved, settings, gate, resolveEffort) },
    { path: ROUTE_STREAM, handler: createStreamHandler(ctx, resolved, settings, gate, resolveEffort) },
    {
      path: ROUTE_CATALOG,
