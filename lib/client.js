...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 设置字段的区间（与宿主 `lib/policy.js` 的 `TEMPERATURE_RANGE` /
     * `MAX_OUTPUT_TOKENS_RANGE` / `TIMEOUT_RANGE` **同值**）。
     *
     * 这里必须带上界：宿主侧 `validate` 会让超界写入被拒，而客户端镜像只查下界时，
     * 用户填 700000 会先通过预校验、再由宿主拒绝——旧代码在这种情况下会假报"已保存"
     * （见 `opsApplied` 的注释）。P5.2 计划把这份区间改由 `/catalog` 下发，届时删掉这份镜像。
     */
