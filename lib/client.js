...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function BetterInputButton(props) {
      ...
      // 预设与区间来自宿主（单一事实来源）：每次页面加载最多一次往返，失败就退回兜底值。
      React.useEffect(() => {
        let live = true
        void catalogShared()
          .then(({ presets: list, limits: fromHost }) => {
      ...
      const onOptimize = async (presetId) => {
        ...
        setRunning(true)
        try {
          const outcome = await requestOptimize({
            text: before,
            sessionId,
          ...
          pushUndo(sessionId, { before, after: outcome.text })
      ...
      // 预设菜单：只有宿主真的配了预设（`cordis.patch.yml` 的 presets）才出现，
      // 所以没配预设的部署视觉与之前完全一致。
      if (presets.length > 0) {
        children.push(React.createElement('button', {
          key: 'preset-toggle',
     ...
     * @returns {Promise<{ limits: object, presets: object[] }>} 规则与预设。
     */
      ...
      if (catalogOnce === undefined) {
        catalogOnce = requestCatalog()
          .then((data) => {
            ...
            return { limits, presets, raw: data }
          })
          .catch((error) => {
