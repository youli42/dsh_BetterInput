...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 进度行 / 思考面板的最小刷新间隔（毫秒，P11）。
     *
     * 思考增量是**逐 token** 到的，每个都 setState 会让这个组件每秒重渲染上百次。
     * 与 `STREAM_WRITE_MS` 同一套思路，只是这里节流的是"显示"而不是"写编辑器"。
     */
    const PROGRESS_MS = 100
    /**
     * 思考文本在客户端最多保留的字符数（保留**尾部**）。
     *
     * 生成中要跟着最新思考走，所以截断留尾；被截断时面板头部仍显示累计总字数，
     * 正文开头加一句"仅保留最近部分"——截断了却不说，用户会以为模型就想这么点。
     */
    ...
    /** 设置段里"是否显示模型的思考过程"的字段名（未设置 = 默认开；见宿主 SHOW_REASONING_FIELD）。 */
    const SHOW_REASONING_FIELD = 'showReasoning'
    /* ── 思考强度（P14，与宿主 lib/policy.js 的字段名一致） ── */
    /** 设置段里"思考强度"的字段名（未设置 = 宿主内置 low）。它归在「调用参数」分组里。 */
    const DEFAULT_REASONING_EFFORT_FIELD = 'defaultReasoningEffort'
    /**
     * 强度 id 的**建议**清单（宿主不可达时的兜底；正常走 `/catalog` 的 `limits.reasoningEfforts`）。
     * 强度集合归适配器所有，这不是白名单——用户填别的值一样合法。
     */
    const REASONING_EFFORT_FALLBACK = ['minimal', 'low', 'medium', 'high']
     ...
     *
     * P12 起只剩这两个：撤销与思考回看都搬进了 ▾ 菜单，菜单项**用文字前缀**（`↶` / `💭`）
     * 而不是图标——菜单项本来就以文字为主，塞 14px 图标既要对齐又要多两条降级路径，
     * 而文字前缀在任何图标集下都长得一样。
     */
    ...
    const zh = {
      ...
      undoEmpty: '没有可撤销的优化',
      'preset.menu': '按预设优化',
      // ── 统一菜单（P12）：工具行只留 ✦ 与 ▾，撤销与思考回看都收进这里 ──
      'menu.label': '优化选项',
      'menu.head.invocation': '本次调用',
      'undo.menuItem': '↶ 撤销上次优化',
      'undo.menuItemDirty': '↶ 撤销上次优化（草稿已改）',
      'undo.menuItemForce': '↶ 强制还原原文',
      'style.concise': '精简',
      'style.spec': '转规格',
      tooLong: '草稿过长，超过宿主上限',
      empty: '输入框是空的，先写点内容',
      ...
      // ── 进度与思考过程（P11）──
      'progress.waiting': '等待模型',
      'progress.thinking': '思考中',
      'progress.writing': '写入中',
      'progress.chars': ' 字',
      'thinking.title': '思考过程',
      'thinking.menuItem': '💭 查看思考过程',
      'thinking.show': '查看思考过程',
      'thinking.hide': '收起思考过程',
      'thinking.truncated': '……（思考过长，这里只保留最近的部分）',

      // ── 通用输入角标（主输入框之外的每个输入窗口）──
      // 角标是 DOM 层的东西，但它和主按钮共用同一套词典与同一套路由文案：
      // 同一句话不该在两个地方有两种说法。
      'badge.optimize': '优化这段输入',
      'badge.running': '生成中，点一次可取消',
      'badge.done': '已替换，点 ↶ 可撤销',
      'badge.undo': '撤销上次优化（还原这段文本）',
      'badge.undoForce': '内容在优化后已被修改：再点一次强制还原',
      ...
      'settings.params.showReasoning': '显示思考过程',
      'settings.params.showReasoningHint': '开启时，优化过程中会在输入框旁显示进度（阶段 / 耗时 / 字数），并在模型返回思考内容时弹出「思考过程」面板。关闭时宿主不把思考内容发到浏览器。没有思考内容的模型只显示进度。',
      // ── 思考强度（P14）：归在「调用参数」里的一个默认值 ──
      'settings.params.reasoningEffort': '思考强度',
      'settings.params.reasoningEffortPlaceholder': '留空 = 内置默认 low',
      'settings.params.reasoningEffortHint': '优化时传给模型的思考强度（reasoning effort）。留空 = 内置默认 low。可用取值由模型自身决定，宿主会把模型不支持的取值回落到它自己的默认值。',
      'settings.save': '保存',
      'settings.saving': '保存中…',
    ...
    const en = {
      ...
      // ── one unified menu (P12): the tool row keeps only ✦ and ▾ ──
      'menu.label': 'Optimize options',
      'menu.head.invocation': 'This run',
      'undo.menuItem': '↶ Undo last optimization',
      'undo.menuItemDirty': '↶ Undo last optimization (draft changed)',
      'undo.menuItemForce': '↶ Force-restore the original',
      ...
      // ── progress and reasoning (P11) ──
      'progress.waiting': 'Waiting for the model',
      'progress.thinking': 'Thinking',
      'progress.writing': 'Writing',
      'progress.chars': ' chars',
      'thinking.title': 'Reasoning',
      'thinking.menuItem': '💭 Show the reasoning',
      'thinking.show': 'Show the reasoning',
      'thinking.hide': 'Hide the reasoning',
      'thinking.truncated': '… (the reasoning is long; only the most recent part is kept)',

      // ── universal input badges (every input window except the main composer) ──
      'badge.optimize': 'Optimize this input',
      'badge.running': 'Generating — click once to cancel',
      'badge.done': 'Replaced — click ↶ to undo',
      'badge.undo': 'Undo the last optimization (restore this text)',
      'badge.undoForce': 'The text changed after optimization: click again to force-restore',
      ...
      'settings.params.showReasoning': 'Show the reasoning',
      'settings.params.showReasoningHint': 'While optimizing, a progress line (phase / elapsed / characters) appears next to the composer, and a "Reasoning" panel opens when the model returns reasoning content. When off, the host does not send reasoning to the browser at all. Models without reasoning only show the progress line.',
      // ── reasoning effort (P14): one default, living in the call parameters ──
      'settings.params.reasoningEffort': 'Reasoning effort',
      'settings.params.reasoningEffortPlaceholder': 'Empty = built-in default "low"',
      'settings.params.reasoningEffortHint': 'The reasoning effort sent to the model while optimizing. Empty means the built-in default "low". The accepted values are model-owned: when a model does not offer the configured value, the host falls back to the model default.',
      'settings.save': 'Save',
    ...
    const CSS = [
      ...
      `position:relative;`,   // 「可撤销」圆点角标（P12）的定位基准
      ...
      // 「可撤销」的角标（P12）：撤销搬进菜单后，菜单收起时也要能看出"上次优化还能撤销"。
      // 用一个 4px 圆点代替一整个按钮——它只表达"有东西可撤"，不承担点击。
      `.dsh-better-input[data-undo="available"]::after{content:'';position:absolute;top:1px;right:1px;`,
      `width:5px;height:5px;border-radius:50%;opacity:.9;`,
      `background:var(--dsw-alias-state-success-primary,currentColor)}`,
      // 生成中的图标必须"看得出在动"（P11）：静止的图标会被当成卡死——那正是加进度行要解决的问题。
      // 只做透明度呼吸（不位移、不缩放），在拥挤的工具行里不会把邻居挤动。
      `.dsh-better-input[data-state="running"] svg{animation:dsh-bi-pulse 1.2s ease-in-out infinite}`,
      `@keyframes dsh-bi-pulse{0%,100%{opacity:.35}50%{opacity:1}}`,
      ...
      // 进度行：与结果提示**同一个槽位**（生成中显示进度，结束后让位给结果提示），所以量取同一套尺寸。
      `.dsh-better-input__progress{min-width:0;max-width:min(240px,28vw);overflow:hidden;text-overflow:ellipsis;`,
      `white-space:nowrap;font-size:11px;color:var(--dsw-alias-label-secondary,currentColor);`,
      `font-variant-numeric:tabular-nums}`,
      // ── 统一菜单（P12）：工具行只留 ✦ 与 ▾，撤销与思考回看都在这里 ──
      // 向上弹出：工具行在输入框下方，上方是对话区，不易被裁剪。
      // 比 P11 时更宽更高（多了一个分区），并**封顶后内部滚动**：菜单变长不能顶穿视口。
      // 分区标题还兼作两类东西的分界：本次调用（撤销 / 思考）vs 长期设置（追加提示词 / 预设）。
      `.dsh-better-input-menu{position:absolute;bottom:calc(100% + 6px);right:0;z-index:30;`,
      `display:flex;flex-direction:column;gap:2px;min-width:120px;max-width:min(280px,72vw);padding:4px;`,
      ...
      // ── 思考过程面板（P11）：与预设菜单同一套定位（按钮上方、右对齐） ──
      // 宽度比菜单大（正文要能读），高度封顶后内部滚动；正文用等宽以外的常规字体，行高放宽。
      `.dsh-better-input-thinking{position:absolute;bottom:calc(100% + 6px);right:0;z-index:31;`,
      `display:flex;flex-direction:column;width:min(420px,72vw);max-height:240px;padding:4px;`,
      `border-radius:10px;background:var(--dsw-alias-bg-layer-2,Canvas);`,
      `border:1px solid var(--dsw-alias-border-l2,color-mix(in srgb,currentColor 20%,transparent));`,
      `box-shadow:0 8px 24px color-mix(in srgb,currentColor 18%,transparent)}`,
      `.dsh-better-input-thinking-head{display:flex;align-items:center;gap:6px;padding:2px 4px 4px;`,
      `font-size:11px;color:var(--dsw-alias-label-tertiary,currentColor)}`,
      `.dsh-better-input-thinking-title{flex:1 1 auto;min-width:0;overflow:hidden;text-overflow:ellipsis;`,
      `white-space:nowrap}`,
      `.dsh-better-input-thinking-close{flex:none;font:inherit;font-size:11px;line-height:1.4;padding:1px 6px;`,
      `border-radius:6px;cursor:pointer;border:1px solid var(--dsw-alias-border-l2,color-mix(in srgb,currentColor 18%,transparent));`,
      `background:transparent;color:var(--dsw-alias-label-secondary,currentColor)}`,
      `.dsh-better-input-thinking-close:hover{color:var(--dsw-alias-label-primary,currentColor)}`,
      `.dsh-better-input-thinking-body{margin:0;flex:1 1 auto;min-height:0;overflow:auto;padding:4px 6px;`,
      `border-radius:6px;white-space:pre-wrap;word-break:break-word;font-size:11px;line-height:1.5;`,
      ...
      // ── 通用输入角标：主输入框之外的输入窗口上的角标（DOM 层，独立于 React 座位）──
      // 容器 fixed + inset:0 但 **pointer-events:none**：它铺满视口却不吃任何点击，
      // 只有真正的角标按钮 pointer-events:auto——覆盖层绝不能挡住宿主页面的交互。
      `.dsh-bi-u-root{position:fixed;inset:0;pointer-events:none;z-index:2147482000}`,
      // 角标：贴在宿主输入框右上角（位置由 JS 用 getBoundingClientRect 算，视口坐标）。
      `.dsh-bi-u-badge{position:absolute;display:none;align-items:center;justify-content:center;`,
      `box-sizing:border-box;width:20px;height:20px;padding:0;border-radius:6px;cursor:pointer;`,
      `pointer-events:auto;font:inherit;font-size:12px;line-height:1;`,
      `color:var(--dsw-alias-label-secondary,currentColor);opacity:.75;`,
      `border:1px solid var(--dsw-alias-border-l2,color-mix(in srgb,currentColor 18%,transparent));`,
      `background:var(--dsw-alias-bg-layer-2,color-mix(in srgb,Canvas 88%,transparent))}`,
      `.dsh-bi-u-badge:hover{opacity:1;color:var(--dsw-alias-label-primary,currentColor)}`,
      `.dsh-bi-u-badge[data-state="idle"]{display:none}`,
      // 生成中要"看得出在动"（与主按钮同一条道理：静止的图标会被当成卡死）。
      `.dsh-bi-u-badge[data-state="running"]{opacity:1;animation:dsh-bi-pulse 1.2s ease-in-out infinite}`,
      `.dsh-bi-u-badge[data-state="ok"]{color:var(--dsw-alias-state-success-primary,currentColor);opacity:.9}`,
      `.dsh-bi-u-badge[data-state="error"]{color:var(--dsw-alias-state-error-primary,currentColor);opacity:1}`,
      // 撤销角标：成功后才出现，贴在优化角标左边。
      `.dsh-bi-u-undo{opacity:.9}`,
      // 提示条：贴在宿主输入框下方（视口坐标）；显示/隐藏由 JS 控制。
      `.dsh-bi-u-note{position:absolute;display:none;max-width:min(300px,60vw);padding:2px 6px;border-radius:6px;`,
      `pointer-events:none;font-size:11px;line-height:1.4;white-space:pre-wrap;word-break:break-word;`,
      `background:var(--dsw-alias-bg-layer-2,color-mix(in srgb,Canvas 92%,transparent));`,
      `border:1px solid var(--dsw-alias-border-l2,color-mix(in srgb,currentColor 14%,transparent));`,
      `color:var(--dsw-alias-label-secondary,currentColor)}`,
      `.dsh-bi-u-note[data-tone="ok"]{color:var(--dsw-alias-state-success-primary,currentColor)}`,
      `.dsh-bi-u-note[data-tone="warn"]{color:var(--dsw-alias-state-warn-primary,currentColor)}`,
      `.dsh-bi-u-note[data-tone="error"]{color:var(--dsw-alias-state-error-primary,currentColor)}`,
    ].join('')
    ...
    /**
     * 进度行文案（P11）：阶段 + 已耗时（+ 该阶段有意义的计数）。
     *
     * **阶段是客户端推出来的，宿主不发进度帧**：`waiting`（已发出请求、还没收到任何事件）
     * → `thinking`（首个思考增量）→ `writing`（首个文本增量）。等待模型那段没有任何事件，
     * 本地时钟是唯一的进度来源，所以耗时由客户端自己算（服务端心跳帧解决的是"连接是否活着"，
     * 那条 fetch/abort 已经覆盖，多一种帧只是多一处会漂移的契约）。
     *
     * 计数用**字数**而不是 token：客户端自己就能算准，且对用户更直观。
     * 不给"百分比"是有意的——`maxOutputTokens` 是上限不是目标，任何比例都是编的。
     * @param {'waiting' | 'thinking' | 'writing'} phase - 当前阶段。
     * @param {number} elapsedMs - 本次调用已耗时（毫秒）。
     * @param {{ reasoning: number, text: number }} counts - 已收到的思考字数与文本字数。
     * @param {(key: string) => string} t - 词典。
     * @returns {string} 一行进度文案。
     */
    function progressText(phase, elapsedMs, counts, t) {
      const seconds = `${(elapsedMs / 1000).toFixed(1)}s`
      if (phase === 'thinking') {
        return `${t('progress.thinking')} ${seconds} · ${String(counts.reasoning)}${t('progress.chars')}`
      }
      if (phase === 'writing') {
        return `${t('progress.writing')} ${seconds} · ${String(counts.text)}${t('progress.chars')}`
      }
      return `${t('progress.waiting')} ${seconds}`
    }
     ...
     *   t: (key: string) => string, onDelta: (delta: string) => void,
     *   onReasoning?: (text: string) => void }} call - 调用参数。
    ...
    async function requestOptimizeStream(call) {
      ...
      for (;;) {
        ...
        while (boundary >= 0) {
          ...
          // 思考增量（P11）：只在生效设置开着时宿主才发；旧宿主/关掉时这一支永远不会命中，
          // 进度行与文本回填照常工作（思考是增强，不是新的失败面）。
          else if (frame.event === 'reasoning' && typeof frame.data?.text === 'string') call.onReasoning?.(frame.data.text)
    ...
    /* ── 通用输入角标：把输入优化覆盖到主输入框之外的每个输入窗口 ──────────────── */

    /**
     * 需求（P13）：「主输入窗口之外的每个输入窗口」都要能优化输入。
     *
     * 为什么走 DOM 而不是座位：座位上只有 ui-conversation 的 composer 有输入状态
     * （`useInput`/`InputState`/`setDraft`）。第三方插件与官方其它输入窗口（自由文本问答、
     * 消息反馈、任务看板表单……）的草稿**不在任何跨插件契约里**——没有座位、没有共享 store、
     * 也无法从外部 `import`（客户端 bundle 只能 require 平台种子模块）。能覆盖"大部分输入窗口"
     * 的只有一个共同契约：它们最终都是页面上的 `<textarea>`。所以这里在 DOM 层做事。
     *
     * 与主按钮的关系：**同一套宿主路由、同一套词典、同一套流式/回退/CAS 语义**，只是
     * 目标从 React 草稿换成 DOM 输入框。主输入框（Lexical contenteditable）不是 textarea，
     * 天然不会被挂上角标——两个入口不会打架。
     *
     * 明确的边界：
     *   · 只挂 `<textarea>`（DSH 里"人话输入"的标准控件）与**显式报名**的元素
     *     （`data-dsh-better-input-host`，给非 textarea 的第三方控件用）；单选/数字输入框不挂。
     *   · 只读、禁用、隐藏（rect 太小或不在视口内）的宿主不显示角标。
     *   · 受控组件兼容：写回走**平台原生 value setter + input 事件**（React 的 value tracker
     *     会忽略直接赋值），否则插件的 React state 会与 DOM 漂移。
     *   · 逃生口：宿主元素或任一祖先带 `data-dsh-better-input-skip` 就不挂。
     */
    /** 默认宿主选择器：多行文本框。 */
    const UNIVERSAL_SELECTOR = 'textarea'
    /** 显式报名：任何带这个属性的元素都当输入窗口。 */
    const UNIVERSAL_HOST_ATTR = 'data-dsh-better-input-host'
    /** 跳过标记：元素自身或祖先带它就不挂角标。 */
    const UNIVERSAL_SKIP_ATTR = 'data-dsh-better-input-skip'
    /** 覆盖层容器标记（容器内不再挂角标，防自噬）。 */
    const UNIVERSAL_ROOT_ATTR = 'data-dsh-bi-universal-root'
    /** 角标 / 撤销角标 / 提示条的元素标记（外部排查脚本按它们定位）。 */
    const UNIVERSAL_BADGE_ATTR = 'data-dsh-better-input-badge'
    const UNIVERSAL_UNDO_ATTR = 'data-dsh-better-input-badge-undo'
    const UNIVERSAL_NOTE_ATTR = 'data-dsh-bi-badge-note'
    /**
     * 这些子树里的输入框不挂角标：
     *   · 本插件自己的界面（主按钮工具行 / 设置页）——那儿的输入框归主按钮与设置页自己管；
     *   · 主输入框（composer）区域——它自己有 ✦ 按钮，不该再多一枚角标。
     *     composer 在本版本是 Lexical 的 contenteditable（本来就不是 textarea），
     *     但旧版本/降级态可能渲染真正的 textarea（例如"输入被阻塞"时的惰性 textarea），
     *     所以这里按框架自己的标记排除，而不是假设"永远不会有 textarea"。
     */
    const UNIVERSAL_EXCLUDE_SELECTORS = [
      '[data-dsh-better-input-wrap]',
      '[data-dsh-bi-settings]',
      '[data-composer-input]',
      '[data-composer-seat]',
    ]
    /** 角标尺寸与内缩（像素）；位置 = 宿主右上角往内缩。 */
    const UNIVERSAL_BADGE_SIZE = 20
    const UNIVERSAL_BADGE_INSET = 4
    /** 一个页面最多挂多少个角标：第三方页面可能有几十个 textarea，无限挂下去只是浪费。 */
    const UNIVERSAL_MAX_HOSTS = 40
    /** 可见性门槛：比这更小/更扁的 textarea 不当成"人话输入窗口"（搜索框之类）。 */
    const UNIVERSAL_MIN_WIDTH = 80
    const UNIVERSAL_MIN_HEIGHT = 28
    /** 扫描去抖：MutationObserver 是成批到达的，每个变更都全量扫描会让输入卡顿。 */
    const UNIVERSAL_SCAN_MS = 50

    /**
     * 读一个 DOM 宿主的当前文本。
     * @param {object} element - 宿主要素。
     * @returns {string} 文本（非字符串一律按空串）。
     */
    function universalHostText(element) {
      const tag = String(element?.tagName ?? '').toUpperCase()
      if (tag === 'TEXTAREA' || tag === 'INPUT') return typeof element.value === 'string' ? element.value : ''
      return typeof element.textContent === 'string' ? element.textContent : ''
    }

    /**
     * 派发一次 `input` 事件（冒泡），让受控组件的 React state 跟上 DOM。
     * @param {object} win - window。
     * @param {object} element - 宿主要素。
     * @returns {void}
     */
    function universalDispatchInput(win, element) {
      let event = null
      try {
        event = typeof win.Event === 'function' ? new win.Event('input', { bubbles: true }) : null
      } catch {
        event = null
      }
      const doc = element.ownerDocument
      if (event === null && doc !== null && doc !== undefined && typeof doc.createEvent === 'function') {
        event = doc.createEvent('Event')
        event.initEvent('input', true, false)
      }
      if (event !== null && typeof element.dispatchEvent === 'function') element.dispatchEvent(event)
    }

    /**
     * 写回一个 DOM 宿主的文本。
     *
     * **必须走平台原生 value setter 并补一个 input 事件**：React 在受控节点上装了
     * value tracker，`element.value = x` 会被它当成"值没变"而**丢掉 onChange**，
     * 于是组件 state 停在旧文本，下一次渲染就把优化结果覆盖回去（看起来像"点了没反应"）。
     * 原生 setter 绕过 tracker、input 事件再把新值送进 onChange——这是社区通行做法。
     * @param {object} win - window。
     * @param {object} element - 宿主要素。
     * @param {string} text - 要写入的文本。
     * @returns {void}
     */
    function universalWriteText(win, element, text) {
      const tag = String(element?.tagName ?? '').toUpperCase()
      if (tag === 'TEXTAREA' || tag === 'INPUT') {
        const proto = tag === 'TEXTAREA' ? win.HTMLTextAreaElement?.prototype : win.HTMLInputElement?.prototype
        const setter = proto === undefined ? undefined : Object.getOwnPropertyDescriptor(proto, 'value')?.set
        if (typeof setter === 'function') setter.call(element, text)
        else element.value = text
      } else {
        element.textContent = text
      }
      universalDispatchInput(win, element)
    }

    /**
     * 造一个通用角标控制器（每个环境一个实例；`apply()` 持有一个，测试可另建）。
     * @param {{ t: (key: string) => string, win?: object, doc?: object }} options - 词典与 DOM 环境。
     * @returns {object | null} 控制器；环境不完整（没有 body / 没有 querySelectorAll）时返回 null。
     */
    function createUniversalBadges(options) {
      const doc = options.doc ?? (typeof document === 'undefined' ? null : document)
      const win = options.win ?? (typeof window === 'undefined' ? null : window)
      if (doc === null || win === null) return null
      if (typeof doc.createElement !== 'function' || typeof doc.querySelectorAll !== 'function') return null
      if (doc.body === null || doc.body === undefined) return null
      const t = options.t

      /** 宿主元素 → 角标记录。 */
      const records = new Map()
      /** 宿主元素 → 可撤销记录（弱引用，元素被移除后可回收）。 */
      const undos = new WeakMap()
      /** 宿主元素 → 合成会话 id（宿主并发闸门的"同会话单航班"键）。 */
       ...
       * 取宿主元素对应的会话键。
       *
       * 通用角标没有会话上下文，但宿主闸门需要"同一个输入窗口不并发两次"。
       * 所以按元素给一个稳定合成 id：**同一窗口单航班，全局上限照旧**（宿主 maxConcurrentCalls）。
       * 元素可以用 `data-dsh-better-input-session` 显式指定（排查时能用上）。
       * @param {object} host - 宿主要素。
       * @returns {string} 会话键。
       */
      const sessionIdOf = (host) => {
        let id = sessionIds.get(host)
        if (id === undefined) {
          nextSessionSeq += 1
          const declared = typeof host.getAttribute === 'function' ? host.getAttribute('data-dsh-better-input-session') : null
          id = typeof declared === 'string' && declared !== '' ? declared : `dom-input-${String(nextSessionSeq)}`
          sessionIds.set(host, id)
        }
        return id
      }
      ...
      const ensureRoot = () => {
        ...
        root = typeof doc.querySelector === 'function' ? doc.querySelector(`[${UNIVERSAL_ROOT_ATTR}]`) : null
        if (root === null || root === undefined) {
          root = doc.createElement('div')
          root.setAttribute(UNIVERSAL_ROOT_ATTR, ENTRY_ID)
          root.setAttribute('class', 'dsh-bi-u-root')
          if (typeof doc.body.append === 'function') doc.body.append(root)
      ...
      /**
       * 造一个角标按钮。
       * @param {string} attr - 定位属性（角标 / 撤销角标）。
       * @param {string} glyph - 文字图标（不依赖图标集：菜单项也是这么处理的）。
       * @param {string} className - 附加 class。
       * @param {string} title - 初始 tooltip。
       * @returns {object} 按钮元素。
       */
      const createBadge = (attr, glyph, className, title) => {
        const badge = doc.createElement('button')
        badge.setAttribute('type', 'button')
        badge.setAttribute(attr, ENTRY_ID)
        badge.setAttribute('class', className)
      ...
      const createNote = () => {
        ...
        note.setAttribute(UNIVERSAL_NOTE_ATTR, ENTRY_ID)
        note.setAttribute('class', 'dsh-bi-u-note')
       ...
       * @returns {object} 记录。
       */
      const addRecord = (host) => {
        const container = ensureRoot()
        const record = {
          host,
          badge: createBadge(UNIVERSAL_BADGE_ATTR, '✦', 'dsh-bi-u-badge', t('badge.optimize')),
          undoBadge: createBadge(UNIVERSAL_UNDO_ATTR, '↶', 'dsh-bi-u-badge dsh-bi-u-undo', t('badge.undo')),
          note: createNote(),
          controller: null,
          state: 'idle',
          undoArmed: false,
        ...
        record.badge.addEventListener('click', () => { void universalOptimize(record) })
        record.undoBadge.addEventListener('click', () => { universalUndo(record) })
        for (const node of [record.badge, record.undoBadge, record.note]) {
          if (typeof container.append === 'function') container.append(node)
      ...
      const removeRecord = (record) => {
        ...
        if (record.noteTimer !== null) win.clearTimeout?.(record.noteTimer)
        for (const node of [record.badge, record.undoBadge, record.note]) node.remove?.()
        records.delete(record.host)
        undos.delete(record.host)
      }

      /**
       * 宿主是否够格挂角标（结构判定；**可见性不在这里判**——那属于定位阶段）。
       *
       * 刻意不在这里要求可见：面板还没展开时 rect 是 0，若因此判定"不够格"，
       * 之后就再也没有第二次机会（MutationObserver 不会为"尺寸变了"再报一次）。
       * @param {object} element - 候选元素。
       * @returns {boolean} 是否够格。
       */
      const isEligible = (element) => {
        if (element === null || typeof element !== 'object') return false
        const tag = String(element.tagName ?? '').toUpperCase()
        const optedIn = typeof element.hasAttribute === 'function' && element.hasAttribute(UNIVERSAL_HOST_ATTR)
        if (tag !== 'TEXTAREA' && !optedIn) return false
        if (element.disabled === true || element.readOnly === true) return false
        if (element.isConnected === false) return false
        if (typeof element.closest === 'function') {
          if (element.closest(`[${UNIVERSAL_SKIP_ATTR}]`) !== null) return false
          if (element.closest(`[${UNIVERSAL_ROOT_ATTR}]`) !== null) return false
          for (const own of UNIVERSAL_EXCLUDE_SELECTORS) {
            if (element.closest(own) !== null) return false
          }
        }
        return true
      }
      ...
      const rectOf = (element) => {
        ...
        return rect !== null && typeof rect === 'object' ? rect : null
      }

      /**
       * 把提示条显示在角标下方（自动隐藏；tone 决定颜色）。
       * @param {object} record - 记录。
       * @param {string} text - 提示文本。
       * @param {'ok' | 'warn' | 'error'} tone - 语气。
       * @returns {void}
       */
      const showNote = (record, text, tone) => {
        record.note.textContent = text
        record.note.setAttribute('data-tone', tone)
        record.note.style.display = 'block'
        record.noteVisible = true
        if (record.noteTimer !== null) win.clearTimeout?.(record.noteTimer)
        record.noteTimer = typeof win.setTimeout === 'function'
          ? win.setTimeout(() => {
            record.noteTimer = null
            record.noteVisible = false
            record.note.style.display = 'none'
      ...
      /**
       * 宿主是不是"真的在最上层"（没被别的元素盖住）。
       *
       * 只看矩形不够：对话框/抽屉盖在页面上时，底下的输入框矩形还在视口里，角标会浮在
       * **对话框之上**——看起来像跑到别人的窗口上去了。用 `elementFromPoint` 取宿主中心点
       * 的命中元素来判断：命中宿主（或它的后代/我们自己的角标层）才算可见。
       * 环境不支持或取不到时**一律当可见**——这个判据是"少画错"的增强，不该变成"少画"的失败面。
       * @param {object} host - 宿主要素。
       * @param {object} rect - 宿主矩形。
       * @returns {boolean} 是否在最上层。
       */
      const isTopmost = (host, rect) => {
        if (typeof doc.elementFromPoint !== 'function') return true
        const x = rect.left + rect.width / 2
        const y = rect.top + rect.height / 2
        if (x < 0 || y < 0 || (typeof win.innerWidth === 'number' && x > win.innerWidth)) return true
        const hit = doc.elementFromPoint(x, y)
        if (hit === null || hit === undefined) return true
        if (hit === host) return true
        if (typeof host.contains === 'function' && host.contains(hit)) return true
        return typeof hit.closest === 'function' && hit.closest(`[${UNIVERSAL_ROOT_ATTR}]`) !== null
      }

      /**
       * 按宿主当前矩形摆放角标。
       *
       * 角标挂在 fixed 覆盖层里，位置每帧都要跟着宿主走：滚动、缩放、DOM 变化都会重算
       * （滚动用捕获阶段监听——输入框常常在内部滚动容器里）。
       * @returns {void}
       */
      const reposition = () => {
        if (disposed) return
        const viewportHeight = typeof win.innerHeight === 'number' ? win.innerHeight : null
        for (const record of records.values()) {
          const rect = rectOf(record.host)
          const usable = rect !== null
            && rect.width >= UNIVERSAL_MIN_WIDTH
            && rect.height >= UNIVERSAL_MIN_HEIGHT
            && (viewportHeight === null || (rect.bottom > 0 && rect.top < viewportHeight))
            && isTopmost(record.host, rect)
          if (!usable) {
            record.badge.style.display = 'none'
            record.undoBadge.style.display = 'none'
            if (record.noteVisible !== true) record.note.style.display = 'none'
            continue
          }
          const size = UNIVERSAL_BADGE_SIZE
          const left = Math.round(rect.right - size - UNIVERSAL_BADGE_INSET)
          const top = Math.round(rect.top + UNIVERSAL_BADGE_INSET)
          record.badge.style.left = `${String(left)}px`
          record.badge.style.top = `${String(top)}px`
          record.badge.style.display = 'flex'
          record.undoBadge.style.left = `${String(left - size - 2)}px`
          record.undoBadge.style.top = `${String(top)}px`
          record.undoBadge.style.display = undos.has(record.host) ? 'flex' : 'none'
          record.note.style.left = `${String(Math.round(rect.left))}px`
          record.note.style.top = `${String(Math.round(rect.bottom + 4))}px`
          record.note.style.display = record.noteVisible === true ? 'block' : 'none'
        }
      }
      ...
      const collectHosts = () => {
        const found = []
        for (const selector of [UNIVERSAL_SELECTOR, `[${UNIVERSAL_HOST_ATTR}]`]) {
          for (const element of doc.querySelectorAll(selector) ?? []) {
            if (found.length >= UNIVERSAL_MAX_HOSTS) return found
            if (!found.includes(element) && isEligible(element)) found.push(element)
          }
        }
        return found
      }

      /** 全量扫描：新宿主挂角标，消失的宿主摘角标。 */
      const scan = () => {
        if (disposed) return
        const hosts = collectHosts()
        for (const [host, record] of [...records]) {
          if (!hosts.includes(host) || host.isConnected === false) removeRecord(record)
        }
        for (const host of hosts) {
          if (!records.has(host)) addRecord(host)
        }
        reposition()
      }

      /** 去抖扫描（MutationObserver 成批到达）。 */
      const scheduleScan = () => {
        if (disposed) return
      ...
      /** 生成中才开的进度计时器：idle 时一个定时器都不留。 */
      const startTicker = () => {
        if (ticker !== null || disposed || typeof win.setInterval !== 'function') return
        ticker = win.setInterval(() => {
          for (const record of records.values()) record.publish?.(true)
        }, PROGRESS_MS)
      }
      ...
      /** 设置角标状态（running / idle / ok / error；样式按它选）。 */
      const setState = (record, state) => {
        record.state = state
        record.badge.setAttribute('data-state', state)
      }

      /** 设置 tooltip。 */
      const setTitle = (badge, text) => {
        badge.setAttribute('title', text)
      }

      /** 显示撤销角标（成功之后才有）。 */
      const showUndo = (record) => {
        setTitle(record.undoBadge, t('badge.undo'))
        record.undoArmed = false
      }

      /**
       * 优化一个 DOM 宿主：读文本 → 宿主路由（流式，不可用回退一次性 JSON）→ 写回。
       *
       * CAS 与主按钮同源：记住"本次调用里我们写过的每一版文本"，当前文本落在集合之外
       * 即认定用户手改 → 立即中止（宿主侧随之取消上游）并提示。
       * @param {object} record - 记录。
       * @returns {Promise<void>} 完成。
       */
      const universalOptimize = async (record) => {
        if (disposed || records.get(record.host) !== record) return
        // 生成中再点 = 取消（与主按钮一致）。
        ...
        const before = universalHostText(host)
        if (before.trim() === '') {
          showNote(record, t('empty'), 'warn')
          return
        }
        let limits = LIMITS_FALLBACK
        try {
          limits = (await catalogShared()).limits ?? LIMITS_FALLBACK
        } catch {
          limits = LIMITS_FALLBACK
        }
        if (disposed || records.get(record.host) !== record) return
        if (before.length > limits.maxInputChars) {
          showNote(record, `${t('tooLong')}（${String(before.length)}/${String(limits.maxInputChars)}）`, 'warn')
          return
        }
        ...
        /** 本次调用里"可能是我们自己造成的"文本集合（原文 + 我们写过的每一版）。 */
        ...
        const publish = (force) => {
          const now = Date.now()
          if (!force && now - lastPublishAt < PROGRESS_MS) return
          lastPublishAt = now
          setTitle(record.badge, progressText(stage, now - record.startedAt, { reasoning: reasoningTotal, text: accumulated.length }, t))
        }
         ...
         * 写回一次（节流；收尾那次 force）。
         *
         * **CAS 判定排在节流之前**：节流只该少写几次，不该少判几次。反过来写的话，
         * "用户改完的 80ms 内又来了一个增量"这一段里，我们的写入会因为节流直接返回"成功"，
         * 手改要等到下一次超出窗口的写入（或收尾那次 force）才被发现——那段时间里
         * 编辑器还在被人改，而我们以为一切正常。
         * @param {string} text - 文本。
         * @param {boolean} force - 忽略节流。
         * @returns {boolean} 是否写成功（false = 用户中途改过）。
         */
        const write = (text, force) => {
          if (!own.has(universalHostText(host))) return false
          const now = Date.now()
          if (!force && now - lastWriteAt < STREAM_WRITE_MS) return true
          universalWriteText(win, host, text)
          own.add(text)
          lastWriteAt = now
          return true
        }
        ...
        const commit = (text, truncated) => {
          undos.set(host, { before, after: text })
          setState(record, 'ok')
          setTitle(record.badge, t('badge.done'))
          showUndo(record)
          if (truncated) showNote(record, t('doneTruncated'), 'warn')
          reposition()
        }
        ...
        try {
          const outcome = await requestOptimizeStream({
            ...
            onReasoning: (text) => {
              reasoningTotal += text.length
              const first = stage !== 'thinking'
              stage = 'thinking'
              publish(first)
            },
          })
          if (disposed || records.get(host) !== record) return
          if (controller.signal.aborted) {
            if (userEdited) showNote(record, t('staleResult'), 'warn')
            return
          }
          if (outcome.unavailable === true) {
            ...
            if (disposed || records.get(host) !== record || controller.signal.aborted) return
            if (!fallback.ok) {
              fail(fallback.message)
              return
            }
            if (universalHostText(host) !== before) {
              showNote(record, t('staleResult'), 'warn')
              return
            }
          ...
          if (!outcome.ok) {
            ...
            fail(partialWritten ? `${outcome.message}（${t('streamReverted')}）` : outcome.message)
            return
          }
          if (!write(outcome.text, true)) {
            showNote(record, t('staleResult'), 'warn')
            return
          }
          commit(outcome.text, outcome.truncated)
        } catch {
          // 取消（用户点第二次，或"检测到用户手改后自己中止"）会以"流被中断"的形式抛到这里，
          // 与正常返回时那条 aborted 分支互为兜底：**两条路都要给出手改提示**，
          // 否则同样一次手改，提示出不出来取决于浏览器把中断报在哪一步。
          if (controller.signal.aborted) {
            if (userEdited) showNote(record, t('staleResult'), 'warn')
          } else {
            if (own.size > 1) write(before, true)
          ...
          record.publish = null
          if (record.state === 'running') setState(record, undos.has(host) ? 'ok' : 'idle')
          if (records.get(host) === record) setTitle(record.badge, t('badge.optimize'))
          stopTickerIfIdle()
        }
      }
       ...
       * 撤销一个宿主上的上次优化。
       *
       * 两步确认与主按钮同语义：文本在优化后被动过时，第一次点击只"武装"
       * （tooltip 换成强制还原文案），再点一次才真的还原。
       * @param {object} record - 记录。
       * @returns {void}
       */
      const universalUndo = (record) => {
        if (disposed || record.controller !== null) return
        const entry = undos.get(record.host)
        if (entry === undefined) return
        const current = universalHostText(record.host)
        const forced = current !== entry.after
        if (forced && record.undoArmed !== true) {
          record.undoArmed = true
          setTitle(record.undoBadge, t('badge.undoForce'))
          showNote(record, t('undoDirty'), 'warn')
          return
        }
        universalWriteText(win, record.host, entry.before)
        undos.delete(record.host)
        record.undoArmed = false
        setState(record, 'idle')
        setTitle(record.badge, t('badge.optimize'))
        reposition()
        showNote(record, forced ? t('undoneForced') : t('undone'), 'ok')
      }
      ...
      const start = () => {
        ensureRoot()
        scan()
        const MutationCtor = win.MutationObserver
        if (typeof MutationCtor === 'function') {
          observer = new MutationCtor(scheduleScan)
          observer.observe(doc.body, { childList: true, subtree: true })
        }
        if (typeof win.addEventListener === 'function') {
          // 捕获阶段：输入框常常在内部滚动容器里，冒泡到 window 的滚动事件收不全。
          win.addEventListener('scroll', reposition, true)
          win.addEventListener('resize', reposition)
        }
      }

      /** 卸载：摘掉全部角标、断开观察器、停掉计时器（插件卸载/HMR 重载时调用）。 */
      const uninstall = () => {
        disposed = true
    ...
    function BetterInputButton(props) {
      ...
      /**
       * 生成中的进度（P11）：`null` = 没在跑。阶段由收到的事件推导（见 `progressText`）。
       * 它同时是"结束/开始"的开关：收尾时置回 null，进度行就让位给结果提示。
       */
      const [progress, setProgress] = React.useState(null)
      /** 本次调用已耗时（毫秒）：本地计时器每 PROGRESS_MS 刷新一次。 */
      const [elapsedMs, setElapsedMs] = React.useState(0)
      /** 模型的思考文本：**只用于展示**，绝不进草稿、不进撤销栈（宿主装配时也只取 text 块）。 */
      const [thinking, setThinking] = React.useState('')
      /** 思考文本的累计字数（面板正文被截断时仍如实显示总数）。 */
      const [thinkingTotal, setThinkingTotal] = React.useState(0)
      /** 思考面板是否展开：出现第一段思考时自动展开一次，收尾自动收起（之后可点图标回看）。 */
      ...
      /** 思考正文容器：新内容到达时滚到底部（真实 DOM 才有这个元素；测试替身里恒为 null）。 */
      const thinkingBody = React.useRef(null)
       ...
       * 生成中的耗时计时器（P11）。
       *
       * 只在 `running` 期间存在：**"等待模型"那段一个事件都没有**，没有本地时钟的话界面就是静止的
       * ——这正是"看起来卡住了"的来源。间隔取 PROGRESS_MS（100ms）：0.1s 的粒度够用，
       * 而 10Hz 的 setState 对这个小节点毫无压力。
       */
      React.useEffect(() => {
        if (!running) return () => {}
        const startedAt = Date.now()
        const id = window.setInterval?.(() => { setElapsedMs(Date.now() - startedAt) }, PROGRESS_MS)
        return () => { if (id !== undefined) window.clearInterval?.(id) }
      }, [running])

      // 思考面板跟随最新内容滚动：真实 DOM 上才有 scrollTop（替身里 ref 一直是 null，跳过即可）。
      React.useEffect(() => {
        const element = thinkingBody.current
        if (element !== null && element !== undefined) element.scrollTop = element.scrollHeight
      }, [thinking, panelOpen])
      ...
      const onOptimize = async (presetId) => {
        ...
        // 新的一次调用顶掉上一次的进度、思考与结果提示：旧提示属于上一次调用，
        // 留着会在"这次被取消/没结果提示"时突然冒出来误导人。
        ...
        setNote(null)
        setElapsedMs(0)
        setProgress({ phase: 'waiting', reasoning: 0, text: 0 })
        setThinking('')
        setThinkingTotal(0)
        setPanelOpen(false)
        setRunning(true)
        ...
        /* ── 本次调用的实时状态（进度行 + 思考面板，P11） ──
         * 都是这一次调用的闭包局部量：进度与思考只属于"当前这次优化"，
         * 结束后最多留下思考文本供回看（React state），不留任何跨调用的残留。
         */
        /** 当前阶段：waiting（等第一个事件）→ thinking（首个思考增量）→ writing（首个文本增量）。 */
        let stage = 'waiting'
        /** 思考文本累计字数（面板正文可能被截断，这个数不受影响）。 */
        let reasoningTotal = 0
        /** 思考文本（只保留尾部 MAX_THINKING_CHARS 字）。 */
        let thinkingText = ''
        /** 上次把实时状态推给界面的时刻（节流用）。 */
        let lastPublishAt = 0
        /** 面板只自动展开**一次**：用户手动收起后，不该被下一段增量顶开。 */
        let panelOpened = false

        /**
         * 把实时状态推给界面（节流）。
         *
         * 为什么节流：思考增量逐 token 到达，每个都 setState 会让这个组件每秒重渲染上百次。
         * 节流只作用于"同一阶段内的计数滚动"；**阶段切换与收尾一律 force**，必须立刻可见。
        ...
        const publish = (force) => {
          ...
          if (!force && now - lastPublishAt < PROGRESS_MS) return
          lastPublishAt = now
          setProgress({ phase: stage, reasoning: reasoningTotal, text: accumulated.length })
          if (stage !== 'thinking') return
          setThinking(thinkingText)
          setThinkingTotal(reasoningTotal)
          if (!panelOpened) {
            panelOpened = true
            setPanelOpen(true)
          }
        ...
        /**
         * 收尾：写入最终文本、压一条撤销记录、必要时提示。
         *
         * **成功不弹提示（P12）**：草稿被替换是肉眼可见的，再闪一句「已替换」属于纯噪声；
         * 「上次优化还能撤销」这件事改由 ▾ 上的圆点角标表达。只有**用户无法从界面推断**的情形才提示，
         * 截断正是这种——结果看起来正常，其实少了内容。
        ...
        const commit = (text, truncated) => {
          ...
          if (truncated) flash(t('doneTruncated'), 'warn')
        }
        ...
        try {
          const outcome = await requestOptimizeStream({
            ...
            onDelta: (delta) => {
              accumulated += delta
              // 阶段切换必须立刻可见（force）；同一阶段内的计数滚动走节流。
              const first = stage !== 'writing'
              stage = 'writing'
              publish(first)
              if (!write(accumulated, false)) {
            ...
            /**
             * 思考增量：只累计到展示用的状态里——**绝不碰草稿**（`write` 只由 `onDelta`/`done`
             * 触发，宿主装配最终文本时也只取 text 块，两头都不给思考内容留进草稿的路径）。
             */
            onReasoning: (text) => {
              reasoningTotal += text.length
              thinkingText = `${thinkingText}${text}`.slice(-MAX_THINKING_CHARS)
              const first = stage !== 'thinking'
              stage = 'thinking'
              publish(first)
            },
          ...
          if (alive.current) {
            // 收尾一定落地一次思考文本：节流可能刚好吞掉最后几段增量（那些内容在局部量里没丢）。
            if (reasoningTotal > 0) {
              setThinking(thinkingText)
              setThinkingTotal(reasoningTotal)
            }
            // 进度行退场、面板自动收起；思考文本留着，由图标回看。
            setProgress(null)
            setPanelOpen(false)
            setRunning(false)
          }
       ...
       *
       * **返回值是给菜单用的（P12）**：撤销 / 强制还原是菜单里的一次性动作，执行成功就该收起菜单；
       * 而"第一次点击只是武装强制还原"时**必须保持菜单展开**，用户才能不重开菜单地再点一次
       * （旧的独立按钮没有这个问题——它一直在那儿）。
       * @returns {boolean} 是否真的执行了还原（false = 没可撤的，或只是武装了强制还原）。
       */
      const onUndo = () => {
        const entry = peekUndo(sessionId)
        if (entry === null) { flash(t('undoEmpty'), 'warn'); return false }
      ...
      /** 实时进度（没拿到时按"等待模型、零计数"兜底：running 但 progress 还没落地的这一帧也要有东西可显示）。 */
      const liveProgress = progress ?? { phase: 'waiting', reasoning: 0, text: 0 }
      /** 面板正文是否被截断（保留尾部时前面被丢掉的字数 > 0）。 */
      const thinkingTruncated = thinkingTotal > thinking.length
      /**
       * 面板标题：`思考过程 · N 字`；生成中顺带带上进度——面板铺开时会盖住按钮旁那行进度，
       * 只留一个静止的标题就又把"有没有在动"这件事丢了。
       */
      const thinkingHeader = `${t('thinking.title')} · ${String(thinkingTotal)}${t('progress.chars')}${
        running ? ` · ${progressText(liveProgress.phase, elapsedMs, liveProgress, t)}` : ''
      }`

      const children = []
      if (running) {
        // 生成中：进度行占住提示槽位（阶段 + 耗时 + 计数）。它会"活着动"，这正是这段 UX 的目的。
        children.push(React.createElement('span', {
          key: 'progress',
          className: 'dsh-better-input__progress',
          'data-dsh-better-input-progress': liveProgress.phase,
        }, progressText(liveProgress.phase, elapsedMs, liveProgress, t)))
      } else if (note !== null) {
        // 结束后：结果提示回到同一个槽位（两者不同时出现，避免挤占工具行的横向空间）。
        children.push(React.createElement('span', {
          key: 'note',
          className: 'dsh-better-input__note',
          'data-dsh-better-input-note': ENTRY_ID,
          'data-tone': note.tone,
        }, note.text))
      }
      ...
      // ▾ 统一菜单（P12）：本插件工具行里除 ✦ 之外的**唯一**控件，承载全部次要功能。
      //
      // 三个分区按"什么时候有用"排列，而不是按实现层次：
      //   本次调用（撤销 / 查看思考）→ 追加提示词（长期偏好，这次用哪一套）→ 预设（本次额外要求）。
      // **入口条件不再只看 /catalog**：撤销与思考属于"上一次调用留下的东西"，
      // 拿不到目录（离线 / 宿主半缺失）时它们照样得能触达——否则一次成功的优化之后，
      // 用户会发现撤销入口随着目录读取失败一起消失了。
      const oneShotPresets = presets.filter(preset => !STYLE_IDS.includes(preset.id)
        && !profiles.some(profile => profile.id === preset.id))
      // 「强制还原」已武装：菜单项文案要如实切换（它同时是"再点一次"的唯一指引）。
      const undoArmed = top !== null && forceArmed.current === top
      /** 本次调用分区里有没有东西可看：有可撤销的，或有思考内容可回看。 */
      const hasInvocation = top !== null || (!running && thinking !== '')
      const showMenu = hasInvocation || profiles.length > 0 || oneShotPresets.length > 0
      if (showMenu) {
        children.push(React.createElement('button', {
          key: 'preset-toggle',
          type: 'button',
          ...
          // 「可撤销」角标：撤销搬进菜单后，菜单收起时这是唯一的提示。
          ...top === null ? {} : { 'data-undo': 'available' },
          ...
          onMouseDown: (event) => { event.preventDefault() },
          // 与思考面板互斥（两者锚在同一处，同时打开会叠在一起）。
          onClick: () => { setMenuOpen(open => !open); setPanelOpen(false) },
        }, '▾'))
        if (menuOpen) {
          const items = []
          if (hasInvocation) {
            ...
            }, t('menu.head.invocation')))
            // 撤销：菜单里的一次性动作。执行成功才收起菜单——"第一次点击只是武装强制还原"时
            // 菜单必须留着，用户才能接着点第二次（这也正是 onUndo 返回布尔值的原因）。
            if (top !== null) {
              items.push(React.createElement('button', {
                ...
                'data-state': undoArmed ? 'armed' : undoClean ? 'clean' : 'dirty',
                'aria-label': t('undo'),
                title: undoClean ? t('undo') : t('undoDirty'),
                onMouseDown: (event) => { event.preventDefault() },
                onClick: () => { if (onUndo()) setMenuOpen(false) },
              }, undoArmed
                ? t('undo.menuItemForce')
                : undoClean ? t('undo.menuItem') : t('undo.menuItemDirty')))
            }
            // 查看思考过程：菜单与面板互斥，所以这里永远是"打开"，不需要切换语义。
            if (!running && thinking !== '') {
              items.push(React.createElement('button', {
                key: 'thinking',
                type: 'button',
                role: 'menuitem',
                className: 'dsh-better-input-menu-item',
                'data-dsh-better-input-thinking-toggle': ENTRY_ID,
                'aria-label': t('thinking.show'),
                title: t('thinking.show'),
                onMouseDown: (event) => { event.preventDefault() },
                onClick: () => { setMenuOpen(false); setPanelOpen(true) },
              }, `${t('thinking.menuItem')}（${String(thinkingTotal)}${t('progress.chars')}）`))
            }
          }
      ...
      // 思考过程面板（P11）：生成中出现第一段思考时自动展开，收尾自动收起；之后从 ▾ 菜单回看。
      // 与菜单同一套定位（按钮上方、右对齐）；两者互斥：开菜单会关面板，开面板会关菜单。
      if (panelOpen && thinking !== '') {
        children.push(React.createElement('div', {
          key: 'thinking-panel',
          className: 'dsh-better-input-thinking',
          role: 'region',
          'aria-label': t('thinking.title'),
          'data-dsh-better-input-thinking': ENTRY_ID,
        }, [
          React.createElement('div', { key: 'head', className: 'dsh-better-input-thinking-head' }, [
            React.createElement('span', {
              key: 'title',
              className: 'dsh-better-input-thinking-title',
              'data-dsh-better-input-thinking-title': ENTRY_ID,
            }, thinkingHeader),
            React.createElement('button', {
              key: 'close',
              type: 'button',
              className: 'dsh-better-input-thinking-close',
              'data-dsh-better-input-thinking-close': ENTRY_ID,
              onMouseDown: (event) => { event.preventDefault() },
          ...
          React.createElement('pre', {
            key: 'body',
            ref: thinkingBody,
            className: 'dsh-better-input-thinking-body',
            'data-dsh-better-input-thinking-body': ENTRY_ID,
          }, thinkingTruncated ? `${t('thinking.truncated')}\n\n${thinking}` : thinking),
        ]))
      }
    ...
    function formFromSnapshot(snapshot) {
      ...
      return {
        ...
        // 思考过程（P11）：**默认开**，只有显式的 false 才关。表单里是布尔，
        // 保存时"勾上" = unset 回默认（见 buildOps）——不往设置文档里塞一个多余的 true。
        showReasoning: value[SHOW_REASONING_FIELD] !== false,
        // 思考强度（P14）：空串 = 未设置 = 用宿主内置默认 low（与"未设置就回落"同一规矩）。
        defaultReasoningEffort: text(value[DEFAULT_REASONING_EFFORT_FIELD]),
    ...
    function validateForm(form, t, limits) {
      ...
      // 思考强度（P14）：留空合法（= 用宿主内置 low），不需要客户端校验——
    ...
    function buildOps(form, snapshot) {
      ...
      // 思考过程（P11）：只有"关"是有效取值，所以关 → set false、开 → unset。
      // 两边都归一到 `false | undefined`，"勾上再保存"不会把默认值额外写进设置文档。
      push(
        SHOW_REASONING_FIELD,
        form.showReasoning === false ? false : undefined,
        base[SHOW_REASONING_FIELD] === false ? false : undefined,
      )
      // 思考强度（P14）：留空 → unset（回落内置 low），与其它字段同一套语义；它是「调用参数」之一。
      push(
        DEFAULT_REASONING_EFFORT_FIELD,
        form.defaultReasoningEffort.trim() === '' ? undefined : form.defaultReasoningEffort.trim(),
        typeof base[DEFAULT_REASONING_EFFORT_FIELD] === 'string' && base[DEFAULT_REASONING_EFFORT_FIELD].trim() !== ''
          ? base[DEFAULT_REASONING_EFFORT_FIELD]
          : undefined,
      )
    ...
    function BetterInputSettings(props) {
      ...
      /* ── 调用参数：摘要 = 四个生效值，展开后是三个数字框 + 思考强度 ── */
      // 思考强度（P14）归在这里：它是"优化时用一个多大的思考强度"的**调用参数**，与温度/上限/超时同类。
      // 建议清单以宿主下发为准（`/catalog` 的 `limits.reasoningEfforts`）；宿主没到才用兜底——
      // 与 limits 同一套思路，避免"客户端放行、宿主不认"的漂移。强度 id 归适配器所有，不是白名单。
      const effortSuggestions = Array.isArray(catalogData?.limits?.reasoningEfforts)
        && catalogData.limits.reasoningEfforts.length > 0
        ? catalogData.limits.reasoningEfforts.filter(id => typeof id === 'string' && id !== '')
        : REASONING_EFFORT_FALLBACK
      const effortListId = 'dsh-bi-reasoning-efforts'
      const effectiveEffort = effective?.reasoningEffort ?? catalogData?.defaults?.reasoningEffort ?? 'low'
      const paramsBadge = [
        ...
        `${t('settings.params.reasoningEffort')} ${String(effectiveEffort)}`,
      ].join(' · ')
      const paramsGroup = group('params', 'settings.group.params', [badge('params', paramsBadge)], [
        ...
        // 思考强度（P14）：留空 = 内置默认 low。输入框带建议值（datalist），也可手填任意适配器 id。
        inlineRow('settings.params.reasoningEffort', textInput(DEFAULT_REASONING_EFFORT_FIELD, {
          list: effortListId,
          placeholder: t('settings.params.reasoningEffortPlaceholder'),
        })),
        React.createElement('span', { className: 'dsh-bi-hint', key: 'reasoning-effort-hint' },
          t('settings.params.reasoningEffortHint')),
        datalist(effortListId, effortSuggestions.map(id => ({ id, name: id }))),
        // 显示思考过程（P11）：唯一的显示偏好。复选框语义 = 默认开。
        React.createElement('label', { className: 'dsh-bi-check', key: 'show-reasoning' },
          React.createElement('input', {
            type: 'checkbox',
            checked: form.showReasoning !== false,
            disabled: !editable,
            'data-dsh-bi-field': SHOW_REASONING_FIELD,
            onChange: event => { update(SHOW_REASONING_FIELD, event.target.checked) },
          }),
          React.createElement('span', null, t('settings.params.showReasoning')),
        ),
        React.createElement('span', { className: 'dsh-bi-hint', key: 'show-reasoning-hint' },
          t('settings.params.showReasoningHint')),
      ])
    ...
    function apply(ctx) {
      ...
      // 通用输入角标（P13）：主输入框之外的每个输入窗口。DOM 层实现，不占座位；
      // 环境不完整（无 body / 无选择器支持）时返回 null，插件其余功能完全不受影响。
      const universal = createUniversalBadges({ t: ctx.locale.bind(NS) })
      if (universal !== null) {
        ctx.effect(() => () => { universal.uninstall() }, 'better-input: universal badges')
      }
    ...
    /**
     * 通用角标控制器的把手（**框架不读它**）。生产路径只由 `apply()` 创建；
     * 单元测试与外部排查脚本用它直接驱动"扫描 / 重排 / 卸载"，不必伪造整个页面。
     */
    exports.universal = {
      create: createUniversalBadges,
      hostSelector: UNIVERSAL_SELECTOR,
      hostAttr: UNIVERSAL_HOST_ATTR,
      skipAttr: UNIVERSAL_SKIP_ATTR,
      rootAttr: UNIVERSAL_ROOT_ATTR,
      badgeAttr: UNIVERSAL_BADGE_ATTR,
      undoAttr: UNIVERSAL_UNDO_ATTR,
      noteAttr: UNIVERSAL_NOTE_ATTR,
    }
